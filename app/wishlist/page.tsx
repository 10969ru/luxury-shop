    ...
    <div className="min-h-screen bg-black text-white p-6 pt-32">
      ...
      {wishItems.length === 0 ? (
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <button 
            onClick={() => wishItems.forEach(p => {
              addToCart(p, 1);
              toggleWishlist(p.id);
            })} 
            className="w-full border border-zinc-500 py-4 mt-8 hover:bg-white hover:text-black transition"
          >
            全てをカートに積載する
          </button>
        </div>
